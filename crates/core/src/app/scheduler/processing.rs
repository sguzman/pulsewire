  ...
  G: RandomSource + 'static
{
  ...
  let now_ms =
    ctx.clock.now_epoch_ms().await;

  let due_started = Instant::now();

  let due = ctx
    .repo
    .due_feeds_for_category(
      category,
      now_ms,
      due_batch_size
    )
  ...
  let extra_headers_by_id =
    ctx.extra_headers_by_id.clone();
  ...
  G: RandomSource
{
  ...
  let stored =
    repo.latest_state(&feed.id).await?;
  ...
  let action =
    LinkState::decide_next_action(
      &state, now_ms
    );

  let watch = watches_by_id
    .get(&feed.id)
    .cloned();
  ...
  match action {
    ...
    crate::domain::link_state::NextAction::DoHead { state } => {
      let record_history = should_record_history(
        &cfg,
        rng.as_ref(),
      )
      .await;
      ...
      let (action_name, res) =
        if matches!(watch.as_ref().map(|w| w.check_method), Some(WatchCheckMethod::Get)) {
          (
            "GET",
            do_get(
              &cfg,
