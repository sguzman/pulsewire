...
pub fn to_link_state(
  ...
  Some(LinkState {
    ...
    base_poll_seconds: row
      ...
      .max(0)
      as u64,
    max_poll_seconds: cfg
      .max_poll_seconds,
    jitter_fraction: cfg
      .jitter_fraction,
    next_action_at_ms: row
      .next_action_at_ms,
    jitter_seconds: row.jitter_seconds,
    note: row.note.clone(),
    consecutive_error_count: row
...
pub async fn should_record_history<
  G: RandomSource
>(
  cfg: &Arc<AppConfig>,
  rng: &G
) -> bool {
  if cfg.state_history_sample_rate
    >= 1.0
  {
    return true;
  }
...
fn parse_error(
  ...
  match s {
    ...
    | "Http4xx" => {
      Some(ErrorKind::Http4xx(0))
    }
    | "Http5xx" => {
      Some(ErrorKind::Http5xx(0))
    }
    | "ParseError" => {
...
fn parse_http_error(
  ...
  if let Some(code) =
    parse_http_code(s, "Http4xx(")
  {
    return Some(ErrorKind::Http4xx(
      code
    ));
  }

  if let Some(code) =
    parse_http_code(s, "Http5xx(")
  {
