...
//! SQLite pool and backfill missing
//! columns.
...
pub async fn ensure_feed_base_poll_column(
  ...
  info!(
    ...
    "Backfilled base_poll_seconds on \
     feeds"
  );
...
pub async fn ensure_feed_state_note_column(
  pool: &SqlitePool
) -> Result<(), String> {
  let has_column: Option<i64> = sqlx::query_scalar(
  ...
  sqlx::query(
    "ALTER TABLE feed_state_current \
     ADD COLUMN note TEXT NULL"
  )
  .execute(pool)
  .await
  .map_err(|e| {
    format!("add note column: {e}")
  })?;

  info!(
    "Added note column to \
     feed_state_current"
  );
...
pub async fn ensure_feed_state_error_count_column(
  ...
  let sql = format!(
    "SELECT 1 FROM \
     ...
     WHERE name = \
     'consecutive_error_count' LIMIT 1"
  );
  ...
  info!(
    table,
    "Added consecutive_error_count \
     column to feed state table"
  );
...
pub async fn ensure_feed_category_column(
  ...
  .await
  .map_err(|e| {
    format!("add category column: {e}")
  })?;

  info!(
    "Added category column to feeds"
  );

  Ok(())
}

pub async fn set_synchronous(
  pool: &SqlitePool,
  mode: &str
