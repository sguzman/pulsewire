...
//! implementing persistence for feeds,
...
impl SqliteRepo {
  pub async fn new(
    ...
    let pool =
      connection::create_pool(db_path)
        .await?;
...
impl Repo for SqliteRepo {
  ...
  async fn upsert_feeds_bulk(
    ...
    feeds::upsert_feeds_bulk(
      &self.pool, feeds, chunk_size,
      zone
    )
    .await
  }
