...
use crate::auth::{auth_user_id, generate_token, hash_password, hash_token, verify_password};
use crate::errors::{map_db_error, ServerError};
use crate::models::{CreateUserRequest, PasswordChangeRequest, PasswordResetConfirm, PasswordResetRequest, PasswordResetResponse, UserResponse};
...
pub async fn delete_user(
    ...
    if rows == 0 {
        return Err(ServerError::new(StatusCode::NOT_FOUND, "user not found"));
    }
    Ok(StatusCode::NO_CONTENT)
}
...
) -> Result<Json<PasswordResetResponse>, ServerError> {
    let username = payload.username.trim();
    if username.is_empty() {
        return Err(ServerError::new(StatusCode::BAD_REQUEST, "username required"));
    }
