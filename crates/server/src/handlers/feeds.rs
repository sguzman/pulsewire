...
use crate::models::{FeedDetail, FeedSummary};
...
pub async fn feed_detail(
    ...
    let row = sqlx::query_as::<_, FeedDetail>(
        "SELECT id, url, domain, category, base_poll_seconds, created_at_ms FROM feeds WHERE id = ?1",
    )
    .bind(&feed_id)
    .fetch_optional(pool)
    .await
