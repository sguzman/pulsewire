...
use crate::models::{
    EntryListResponse, EntrySummary, FolderCreateRequest, FolderEntriesQuery,
    FolderFeedRequest, FolderFeedRow, FolderFeedUnreadCount, FolderRow, FolderUnreadCount,
    FolderUpdateRequest,
};
...
pub async fn list_folder_entries(
    ...
    headers: HeaderMap,
    AxumPath(folder_id): AxumPath<i64>,
    Query(query): Query<FolderEntriesQuery>,
    ...
    if let Some(pool) = &state.postgres {
        ...
        builder.push(" WHERE ff.folder_id = ");
    ...
    let mut builder = QueryBuilder::<Sqlite>::new(
        "SELECT fi.id, fi.feed_id, fi.title, fi.link,         fi.published_at_ms,         (es.read_at IS NOT NULL) AS is_read         FROM folder_feeds ff         JOIN feed_items fi ON fi.feed_id = ff.feed_id         JOIN folders f ON f.id = ff.folder_id         LEFT JOIN entry_states es ON es.item_id = fi.id AND es.user_id = ",
    );
    builder.push_bind(user_id);
    builder.push(" WHERE ff.folder_id = ");
    builder.push_bind(folder_id);
...
pub async fn folder_feed_unread_counts(
    ...
    headers: HeaderMap,
    AxumPath(folder_id): AxumPath<i64>,
) -> Result<Json<Vec<FolderFeedUnreadCount>>, ServerError> {
    let user_id = auth_user_id(&state, &headers).await?;
    ...
    if let Some(pool) = &state.postgres {
        ...
        let query = format!(
            "SELECT fi.feed_id, COUNT(*)::BIGINT AS unread_count 
             ...
             WHERE ff.folder_id = $2 AND f.user_id = $1 AND es.read_at IS NULL 
             GROUP BY fi.feed_id 
             ORDER BY fi.feed_id",
            quote_ident(schema)
        );
    ...
    let pool = state
        ...
        .as_ref()
        .ok_or_else(|| ServerError::new(StatusCode::INTERNAL_SERVER_ERROR, "database pool missing"))?;
    let rows = sqlx::query_as::<_, FolderFeedUnreadCount>(
